...
epochs = 100
path = "/media/2/Network/extracted_feature/whole_shuffle_to_19_no-rescale/with_"+str(drop_num)+"_packet_error/"
training_result_file =  "./result/result_back_1_adam_"+str(drop_num)+"_packet.txt"
...
val_data = np.load(path+"val_feature_"+str(drop_num)+".npy",mmap_mode="r")
val_label = np.load(path+"val_label_"+str(drop_num)+".npy",mmap_mode="r")
...
#    back_layer = Sequential([layer for layer in back_layer.layers[19:]])
...
#    exit()
    ...
    for i in range(epochs):
        back_layer.fit(train_data,train_label, validation_data=(val_data,val_label), epochs=1, verbose=1,batch_size=256)
...
#            model_save_file = "_epoch_"+str(drop_num)+"_packet_drop_layer1_training.h5"
            ...
            back_layer.save("/media/3/Network/retrain_model_dir/pooling4/1_layer/"+model_save)
        gc.collect()
