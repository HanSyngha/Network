...
for Packet_index in range(0,(65-Packet_number_to_erase)):
    ...
    path = "/media/1/Network/VGG16/weights/bais/1/"+str(Packet_index+1)+"_100.h5"
    ...
    back_layers = Sequential([layer for layer in back_layer_original.layers[:2]])
    back_layers.layers[1].activation = None
    back_layers.add(add_layer)
    back_layers.add(layers.Activation(activations.relu))
    back_layers.add(back_layer_original.layers[2])
    ...
    for Epoch in range(1,11):
        ...
        file_name = "/media/1/Network/VGG16/weights/bais/1/"+str(Packet_index+1)+"_"+str(Epoch * 10)+".h5"
        ...
        back_layers.fit(training_feature_with_error,training_label,batch_size=64,epochs=10,validation_data = (validation_feature_with_error,validation_label),validation_batch_size=64)
        gc.collect()
