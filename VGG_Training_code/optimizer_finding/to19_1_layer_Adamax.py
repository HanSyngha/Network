...
f = open("result_back_1_Adamax.txt","w")
f.close()
for val_lr in [0.1,0.005,0.001,0.002,0.0001,0.00005]:
    ...
    optimizer = tf.keras.optimizers.Adamax(lr=val_lr,beta_1=0.9,beta_2=0.999,epsilon=None, decay=0)
    ...
    for i in tqdm(range(10)):
        ...
        #f = open("result_back_1_adadelta.txt","w")
        f = open("result_back_1_Adamax.txt","a")
        ...
        if i != 0 and i %3 == 0:
            back_layer.save("/media/3/Network/retrain_model_dir/pooling4/1_layer/"+"Adamax"+str(i)+"_back_layer_1_training.h5")
    gc.collect()
gc.collect()
